...
void Renderer::Initialize(int windowSizeX, int windowSizeY)
{
	...
	m_PokeTexture = CreatePngTexture("./poke.png", GL_NEAREST);
...
void Renderer::CreateVertexBufferObjects()
{
		...
		=
	{
		...
		(0.f - temp) * size, (0.f - temp) * size, 0.f, 1.f,
		(1.f - temp) * size, (1.f - temp) * size, 0.f, 1.f,
		(0.f - temp) * size, (1.f - temp) * size, 0.f, 1.f  // Quad 2
	};
...
void Renderer::DrawGridMesh()
{
	...
	glBindTexture(GL_TEXTURE_2D, m_PokeTexture);
...
void Renderer::DrawTexture(float x, float y, float sx, float sy, 
...
void Renderer::DrawParticle()
{
	...
	int aVelocity = glGetAttribLocation(shader, "a_Velocity");
	...
	glVertexAttribPointer(aColLoc, 4, GL_FLOAT, GL_FALSE, sizeof(float) * stride, (GLvoid*)(sizeof(float) * 4));
...
{
	int floatCountPerVertex = 3 + 1 + 4 + 1 + 3 + 1 + 1 + 1 + 1 + 1;  
	...
	int totalVerticesCount = numParticle * floatCountPerParticle;		// 확인필요
	...
	for (int i = 0; i < numParticle; ++i) {
		...
		float vx, vy, vz;
		vx = 0.f; // ((((float)rand() / (float)RAND_MAX) * 2.f - 0.7) * 1.5f);
		vy = 0.f; // ((((float)rand() / (float)RAND_MAX) * 2.f - 1.f) + 1.5f) * 1.5;
		vz = 0.f;
...
void Renderer::CreateFBOs()
{
	...
	//Attach to FB0
	glBindFramebuffer(GL_FRAMEBUFFER, m_FBO0); // attach전에 bind!
	...
	//Attach to FB01
	glBindFramebuffer(GL_FRAMEBUFFER, m_FBO1); // attach전에 bind!
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
		GL_TEXTURE_2D, m_RT1_0, 0);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1,
		GL_TEXTURE_2D, m_RT1_1, 0);
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
		GL_RENDERBUFFER, depthBuffer);
	...
	//Attach to FB0
	glBindFramebuffer(GL_FRAMEBUFFER, m_HDRFBO0); // attach전에 bind!
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
